...
<html>
    ...
    <body>
        <table class="bd" width="100%"><tr><td class="hr"><h2>AgendaRS - [e-mail]</h2></td></tr></table>
        <table width="100%">
            <tr>
...
function select() {
    ...
    <form action="email.php" method="post">
        ...
        echo "selected";
            ...
            <tr>
                ...
                <td><a href="email.php?a=reset">Limpar Filtro</a></td>
            </tr>
        </table>
    </form>
    <hr size="1" noshade>
    ...
    for ($i = $startrec; $i < $reccount; $i++) {
        ...
        ?>
            <tr>
                <td class="<?php echo $style ?>"><a href="email.php?a=view&recid=<?php echo $i ?>">Ver</a></td>
                <td class="<?php echo $style ?>"><a href="email.php?a=edit&recid=<?php echo $i ?>">Editar</a></td>
                <td class="<?php echo $style ?>"><a href="email.php?a=del&recid=<?php echo $i ?>">Excluir</a></td>
                <td class="<?php echo $style ?>"><?php echo htmlspecialchars($row["codigoEmail"]) ?></td>
                <td class="<?php echo $style ?>"><?php echo htmlspecialchars($row["lp_codigoTipoEmail"]) ?></td>
...
function showroweditor($row, $iseditmode) {
    ...
    <table class="tbl" border="0" cellspacing="1" cellpadding="5"width="50%">
        ...
        <tr>
            <td class="hr"><?php echo htmlspecialchars("Endere�o") . "&nbsp;" ?></td>
    ...
    <table class="bd" border="0" cellspacing="1" cellpadding="4">
        <tr>
            <td><a href="email.php?a=add">Inserir Registro</a>&nbsp;</td>
    <?php if ($page > 1) { ?>
                <td><a href="email.php?page=<?php echo $page - 1 ?>">&lt;&lt;&nbsp;Anterior</a>&nbsp;</td>
    <?php } ?>
    <?php
    global $pagerange;
        ...
        for ($i = 1; $i < $rangecount + 1; $i++) {
            ...
            if ((($page >= $startpage) && ($page <= ($i * $pagerange)))) {
                for ($j = $startpage; $j < $count + 1; $j++) {
                    ...
                    <?php } else { ?>
                                <td><a href="email.php?page=<?php echo $j ?>"><?php echo $j ?></a></td>
                    <?php }
                }
            } else { ?>
                        <td><a href="email.php?page=<?php echo $startpage ?>"><?php echo $startpage . "..." . $count ?></a></td>
                <?php }
            }
        } ?>
        <?php if ($page < $pagecount) { ?>
                <td>&nbsp;<a href="email.php?page=<?php echo $page + 1 ?>">Pr�ximo&nbsp;&gt;&gt;</a>&nbsp;</td>
        <?php } ?>
        </tr>
    </table>
<?php } ?>
...
function showrecnav($a, $recid, $count) {
    ...
    <table class="bd" border="0" cellspacing="1" cellpadding="4">
        <tr>
            <td><a href="email.php">P�gina Inicial</a></td>
    <?php if ($recid > 0) { ?>
                <td><a href="email.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>">Registro Anterior</a></td>
    <?php } if ($recid < $count - 1) { ?>
                <td><a href="email.php?a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>">Pr�ximo Registro</a></td>
    <?php } ?>
        </tr>
    </table>
    <hr size="1" noshade>
...
function viewrec($recid) {
    ...
    <table class="bd" border="0" cellspacing="1" cellpadding="4">
        <tr>
            <td><a href="email.php?a=add">Inserir Registro</a></td>
            <td><a href="email.php?a=edit&recid=<?php echo $recid ?>">Editar Registro</a></td>
            <td><a href="email.php?a=del&recid=<?php echo $recid ?>">Excluir Registro</a></td>
        </tr>
    </table>
    <?php
    mysql_free_result($res);
...
function sql_select() {
    ...
    if (!$wholeonly && isset($wholeonly) && $filterstr != '')
        $filterstr = "%" . $filterstr . "%";
    $sql = "SELECT * FROM (SELECT t1.`codigoEmail`, t1.`codigoTipoEmail`, lp1.`descricao` AS `lp_codigoTipoEmail`, t1.`endereco` FROM `email` AS t1 LEFT OUTER JOIN `tipoemail` AS lp1 ON (t1.`codigoTipoEmail` = lp1.`codigoTipoEmail`)) subq";
...
function sql_insert() {
    ...
    $sql = "insert into `email` (`codigoTipoEmail`, `endereco`) values (" . sqlvalue(@$_POST["codigoTipoEmail"], false) . ", " . sqlvalue(@$_POST["endereco"], true) . ")";
...
function sql_update() {
    ...
    $sql = "update `email` set `codigoTipoEmail`=" . sqlvalue(@$_POST["codigoTipoEmail"], false) . ", `endereco`=" . sqlvalue(@$_POST["endereco"], true) . " where " . primarykeycondition();
    mysql_query($sql, $conn) or die(mysql_error());
}
...
function sql_delete() {
    ...
    $sql = "delete from `email` where " . primarykeycondition();
    mysql_query($sql, $conn) or die(mysql_error());
}
