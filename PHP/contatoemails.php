...
<html>
    <head>
        <title>AgendaRS - [Contato (e-mails)]</title>
        <meta name="generator" http-equiv="content-type" content="text/html">
        <link rel="stylesheet" type="text/css" href="style.css">
    </head>
    <body>
        <table class="bd" width="100%"><tr><td class="hr"><h2>AgendaRS - [Contato (e-mails)]</h2></td></tr></table>
        <table width="100%">
            <tr>
...
function select() {
    ...
    ?>><?php echo htmlspecialchars("Contato") ?></option>
                        <option value="<?php echo "lp_codigoEmail" ?>"<?php
                            if ($filterfield == "lp_codigoEmail") {
                                echo "selected";
                            }
    ?>><?php echo htmlspecialchars("e-mail") ?></option>
                    </select></td>
            ...
            <tr>
                ...
                <td><a href="contatoemails.php?a=reset">Limpar Filtro</a></td>
            </tr>
        </table>
    </form>
    <hr size="1" noshade>
    ...
    <table class="tbl" border="0" cellspacing="1" cellpadding="5"width="100%">
        <tr>
            ...
            <td class="hr">&nbsp;</td>
            <td class="hr">&nbsp;</td>
            <td class="hr"><a class="hr" href="contatoemails.php?order=<?php echo "lp_codigoContato" ?>&type=<?php echo $ordtypestr ?>"><?php echo htmlspecialchars("Contato") ?></a></td>
            <td class="hr"><a class="hr" href="contatoemails.php?order=<?php echo "lp_codigoEmail" ?>&type=<?php echo $ordtypestr ?>"><?php echo htmlspecialchars("e-mail") ?></a></td>
        </tr>
        <?php
        for ($i = $startrec; $i < $reccount; $i++) {
            ...
            ?>
            <tr>
                <td class="<?php echo $style ?>"><a href="contatoemails.php?a=view&recid=<?php echo $i ?>">Ver</a></td>
                <td class="<?php echo $style ?>"><a href="contatoemails.php?a=edit&recid=<?php echo $i ?>">Editar</a></td>
                <td class="<?php echo $style ?>"><a href="contatoemails.php?a=del&recid=<?php echo $i ?>">Excluir</a></td>
                <td class="<?php echo $style ?>"><?php echo htmlspecialchars($row["lp_codigoContato"]) ?></td>
                <td class="<?php echo $style ?>"><?php echo htmlspecialchars($row["lp_codigoEmail"]) ?></td>
            </tr>
            <?php
        }
...
function showrow($row, $recid) {
    ...
    <table class="tbl" border="0" cellspacing="1" cellpadding="5"width="50%">
        ...
        <tr>
            ...
            <td class="dr"><?php echo htmlspecialchars($row["lp_codigoEmail"]) ?></td>
        </tr>
    </table>
<?php } ?>
...
function showpagenav($page, $pagecount) {
    ...
    <table class="bd" border="0" cellspacing="1" cellpadding="4">
        <tr>
            <td><a href="contatoemails.php?a=add">Inserir Registro</a>&nbsp;</td>
            <?php if ($page > 1) { ?>
                <td><a href="contatoemails.php?page=<?php echo $page - 1 ?>">&lt;&lt;&nbsp;Anterior</a>&nbsp;</td>
            <?php } ?>
            <?php
            global $pagerange;
                ...
                for ($i = 1; $i < $rangecount + 1; $i++) {
                    ...
                    if ((($page >= $startpage) && ($page <= ($i * $pagerange)))) {
                        for ($j = $startpage; $j < $count + 1; $j++) {
                            ...
                            <?php } else { ?>
                                <td><a href="contatoemails.php?page=<?php echo $j ?>"><?php echo $j ?></a></td>
                                <?php
                            }
                        }
                    } else {
                        ?>
                        <td><a href="contatoemails.php?page=<?php echo $startpage ?>"><?php echo $startpage . "..." . $count ?></a></td>
                        <?php
                    }
                }
            }
            ...
            <?php if ($page < $pagecount) { ?>
                <td>&nbsp;<a href="contatoemails.php?page=<?php echo $page + 1 ?>">Pr�ximo&nbsp;&gt;&gt;</a>&nbsp;</td>
            <?php } ?>
        </tr>
    </table>
<?php } ?>
...
function showrecnav($a, $recid, $count) {
    ...
    <table class="bd" border="0" cellspacing="1" cellpadding="4">
        <tr>
            <td><a href="contatoemails.php">P�gina Inicial</a></td>
            <?php if ($recid > 0) { ?>
                <td><a href="contatoemails.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>">Registro Anterior</a></td>
            <?php } if ($recid < $count - 1) { ?>
                <td><a href="contatoemails.php?a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>">Pr�ximo Registro</a></td>
            <?php } ?>
        </tr>
    </table>
    <hr size="1" noshade>
...
function addrec() {
    ...
    <hr size="1" noshade>
    <form enctype="multipart/form-data" action="contatoemails.php" method="post">
...
function viewrec($recid) {
    ...
    <table class="bd" border="0" cellspacing="1" cellpadding="4">
        <tr>
            <td><a href="contatoemails.php?a=add">Inserir Registro</a></td>
            <td><a href="contatoemails.php?a=edit&recid=<?php echo $recid ?>">Editar Registro</a></td>
            <td><a href="contatoemails.php?a=del&recid=<?php echo $recid ?>">Excluir Registro</a></td>
        </tr>
    </table>
    <?php
    mysqli_free_result($res);
...
function editrec($recid) {
    ...
    <form enctype="multipart/form-data" action="contatoemails.php" method="post">
        ...
        <input type="hidden" name="xcodigoEmail" value="<?php echo $row["codigoEmail"] ?>">
...
function sql_select() {
    ...
    if (!$wholeonly && isset($wholeonly) && $filterstr != '')
        $filterstr = "%" . $filterstr . "%";
    $sql = "SELECT * FROM (SELECT t1.`codigoContato`, lp0.`nome` AS `lp_codigoContato`, t1.`codigoEmail`, lp1.`endereco` AS `lp_codigoEmail` FROM `contatoemails` AS t1 LEFT OUTER JOIN `contato` AS lp0 ON (t1.`codigoContato` = lp0.`codigoContato`) LEFT OUTER JOIN `email` AS lp1 ON (t1.`codigoEmail` = lp1.`codigoEmail`)) subq";
...
function sql_getrecordcount() {
    ...
    if (!$wholeonly && isset($wholeonly) && $filterstr != '')
        $filterstr = "%" . $filterstr . "%";
    $sql = "SELECT COUNT(*) FROM (SELECT t1.`codigoContato`, lp0.`nome` AS `lp_codigoContato`, t1.`codigoEmail`, lp1.`endereco` AS `lp_codigoEmail` FROM `contatoemails` AS t1 LEFT OUTER JOIN `contato` AS lp0 ON (t1.`codigoContato` = lp0.`codigoContato`) LEFT OUTER JOIN `email` AS lp1 ON (t1.`codigoEmail` = lp1.`codigoEmail`)) subq";
    if (isset($filterstr) && $filterstr != '' && isset($filterfield) && $filterfield != '') {
        $sql .= " where " . sqlstr($filterfield) . " like '" . $filterstr . "'";
    } elseif (isset($filterstr) && $filterstr != '') {
        $sql .= " where (`lp_codigoContato` like '" . $filterstr . "') or (`lp_codigoEmail` like '" . $filterstr . "')";
...
function primarykeycondition() {
    ...
    $pk .= ") and ";
    $pk .= "(`codigoEmail`";
    if (@$_POST["xcodigoEmail"] == "") {
        $pk .= " IS NULL";
    } else {
        $pk .= " = " . sqlvalue(@$_POST["xcodigoEmail"], false);
    };
    $pk .= ")";
    return $pk;
}
